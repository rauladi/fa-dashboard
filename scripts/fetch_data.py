...
# ---------- Direct Yahoo Finance API fetch ----------
YAHOO_HEADERS = {"User-Agent": "Mozilla/5.0"}
YAHOO_BASE = "https://query1.finance.yahoo.com"

def yahoo_get_json(endpoint, ticker, period1="0", period2="9999999999", interval="3mo", events=""):
...
def yahoo_fetch_latest_annual_income(ticker_str):
    ...
    if json_q:
        ...
        if len(sheets) >= 4:
            total = {}
            for field in ["totalRevenue","grossProfit","netIncome","basicEPS","dilutedEPS","dividendPerShare"]:
                total[field] = sum(yahoo_extract_field(s, field, fuzzy=[field.lower()]) or 0 for s in sheets[:4])
            return total
    return {}

def yahoo_fetch_latest_quarterly_balance(ticker_str):
...
def yahoo_fetch_2025(sym, ticker_str, target_cur, exchange, usd_aud, usd_idr, twd_usd):
    """Build row_2025 using Yahoo API."""
    fin_cur = financial_currency(exchange)
    div, total_fx, ps_fx = get_fx(target_cur, fin_cur, usd_aud, usd_idr, twd_usd)

    inc = yahoo_fetch_latest_annual_income(ticker_str)
    bal = yahoo_fetch_latest_quarterly_balance(ticker_str)

    row = {f: None for f in FIELDS}

    if inc:
        row["revenue"]     = safe(yahoo_extract_field(inc, "totalRevenue", fuzzy=["revenue"]), div, total_fx)
        row["grossProfit"] = safe(yahoo_extract_field(inc, "grossProfit", fuzzy=["gross"]), div, total_fx)
        row["netProfit"]   = safe(yahoo_extract_field(inc, "netIncome", fuzzy=["net income"]), div, total_fx)
        row["eps"]         = safe(yahoo_extract_field(inc, "basicEPS", "dilutedEPS", fuzzy=["eps"]), 1, ps_fx)
        row["dps"]         = safe(yahoo_extract_field(inc, "dividendPerShare", fuzzy=["dividend"]), 1, ps_fx)

    if bal:
        row["totalAsset"]  = safe(yahoo_extract_field(bal, "totalAssets", "totalAsset", fuzzy=["asset"]), div, total_fx)
        row["cash"]        = safe(yahoo_extract_field(bal, "cashAndCashEquivalents", "cash", fuzzy=["cash"]), div, total_fx)
        row["totalDebt"]   = safe(yahoo_extract_field(bal, "totalDebt", "longTermDebt", fuzzy=["debt","utang"]), div, total_fx)
        row["totalEquity"] = safe(yahoo_extract_field(bal, "totalStockholderEquity", "totalEquity", fuzzy=["equity","ekuitas"]), div, total_fx)
