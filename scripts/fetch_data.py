...
# Optional Gemini AI – will work even if not installed (just skips AI)
try:
    import google.generativeai as genai
    GEMINI_AVAILABLE = True
except ImportError:
    GEMINI_AVAILABLE = False
    print("Gemini not installed. AI insights disabled.", flush=True)
...
def cagr(arr):
    valid = [v for v in arr if v is not None and v != 0]
...
# ---------- SECTOR MAPPING (used for peer comparison) ----------
CO_INFO = {
    "BHP": {"sector": "Mining"},
    "WDS": {"sector": "Oil & Gas"},
    "CBA": {"sector": "Banking"},
    "BBRI": {"sector": "Banking"},
    "ADRO": {"sector": "Coal Mining"},
    "SMSM": {"sector": "Auto Parts"},
    "UNTR": {"sector": "Heavy Equipment & Mining"},
    "ITMG": {"sector": "Coal Mining"},
    "POWR": {"sector": "Power & Utilities"},
    "MPMX": {"sector": "Automotive"},
    "BTPS": {"sector": "Islamic Banking"},
    "DMAS": {"sector": "Industrial Real Estate"},
    "SPTO": {"sector": "Building Materials"},
    "TSM": {"sector": "Semiconductors"},
    "V": {"sector": "Payments"},
    "MA": {"sector": "Payments"},
    "PBR-A": {"sector": "Oil & Gas"},
    "MSFT": {"sector": "Technology"},
    "AMZN": {"sector": "E-Commerce & Cloud"},
    "AAPL": {"sector": "Technology"},
    "META": {"sector": "Social Media"},
    "NVDA": {"sector": "Semiconductors"},
    "GOOG": {"sector": "Technology"},
    "BKNG": {"sector": "Online Travel"},
    "NAB": {"sector": "Banking"},
    "CVX": {"sector": "Oil & Gas"},
    "AXP": {"sector": "Payments"},
    "BAC": {"sector": "Banking"},
...
def compute_sector_medians(stocks_data):
    """Calculate median ratios per sector from the fetched data."""
    sector_metrics = {}
    for sym, d in stocks_data.items():
        sector = d.get('sector', 'General')
        if sector not in sector_metrics:
            sector_metrics[sector] = {'roe': [], 'npm': [], 'de': [], 'rev_cagr': []}
        roe = avg_ratio(d.get('netProfit',[]), d.get('totalEquity',[]))
        npm = avg_ratio(d.get('netProfit',[]), d.get('revenue',[]))
        de = avg_ratio(d.get('totalDebt',[]), d.get('totalEquity',[]))
        rev_cagr_val = cagr(d.get('revenue',[]))
        if roe is not None: sector_metrics[sector]['roe'].append(roe)
        if npm is not None: sector_metrics[sector]['npm'].append(npm)
        if de is not None: sector_metrics[sector]['de'].append(de)
        if rev_cagr_val is not None: sector_metrics[sector]['rev_cagr'].append(rev_cagr_val)
    
    medians = {}
    for sec, metrics in sector_metrics.items():
        medians[sec] = {
            'roe': statistics.median(metrics['roe']) if metrics['roe'] else None,
            'npm': statistics.median(metrics['npm']) if metrics['npm'] else None,
            'de': statistics.median(metrics['de']) if metrics['de'] else None,
            'rev_cagr': statistics.median(metrics['rev_cagr']) if metrics['rev_cagr'] else None
        }
    return medians

# ---------- AI INSIGHTS (Gemini) ----------
def get_ai_insights(sym, data, sector_medians=None):
    """Generate AI summary using Gemini free tier (if available)."""
    if not GEMINI_AVAILABLE:
        return "AI insights require `google-generativeai` package. Install with `pip install google-generativeai` and set GEMINI_API_KEY environment variable."
    
    api_key = os.environ.get('GEMINI_API_KEY')
    if not api_key:
        return "AI insights unavailable: missing GEMINI_API_KEY environment variable."
    ...
    try:
        ...
        # Extract key metrics
        rev_cagr_val = cagr(data.get('revenue',[])) or 0
        np_cagr_val = cagr(data.get('netProfit',[])) or 0
        avg_roe_val = avg_ratio(data.get('netProfit',[]), data.get('totalEquity',[])) or 0
        avg_npm_val = avg_ratio(data.get('netProfit',[]), data.get('revenue',[])) or 0
        avg_de_val = avg_ratio(data.get('totalDebt',[]), data.get('totalEquity',[])) or 0
        
        # Peer context
        peer_text = ""
        if sector_medians and data.get('sector'):
            sec = data['sector']
            med = sector_medians.get(sec, {})
            if med.get('roe'):
                peer_text = f"Sector ({sec}) medians: ROE {med['roe']:.1f}%, NPM {med['npm']:.1f}%, D/E {med['de']:.1f}%."
        
        prompt = f"""For stock {sym}, based on latest 5 years of financial data:
- Revenue CAGR: {rev_cagr_val:.1f}%
- Net Profit CAGR: {np_cagr_val:.1f}%
- Avg ROE: {avg_roe_val:.1f}%
- Avg Net Profit Margin: {avg_npm_val:.1f}%
- Avg Debt/Equity: {avg_de_val:.1f}%
{peer_text}

Write a concise investment thesis (max 120 words) covering:
- 2-3 strongest points
- 1-2 key risks
- A simple verdict (e.g., "Attractive for long-term", "Caution warranted", "Hold")
Use bullet points. Keep it factual and balanced."""
...
# ---------- DEEP STATIC PROFILES (unchanged from original) ----------
PROFILES = {
...
AWS cloud enables faster AI integration. OpenAI partnership driving AI products. Interest rate cuts risk NIM compression but boost mortgage demand. Watch tech execution, competition, and credit quality.""",
    # ... (other profiles truncated for brevity; you must keep the full PROFILES dict from your original file)
    # IMPORTANT: Your original PROFILES dict for all 28 stocks must be placed here.
    # For space, I've shown only a few. Please copy your existing PROFILES from your current fetch_data.py.
}

# Leadership data (same as original, but we keep a placeholder)
LEADERSHIP = {
    ...
    # ... (copy your full LEADERSHIP dict from original file)
}
...
def generate_static_profiles(out):
    for sym, st_data in out["stocks"].items():
        ...
        # dummy m object for compatibility
        m = type('', (), {})()
        m.cg = type('', (), {})()
        ...
        st_data["profile"] = profile_text
        st_data["profileDate"] = NOW.isoformat()
        st_data["news"] = "For latest news, please refer to company announcements and recent filings."
        st_data["newsDate"] = NOW.isoformat()
...
def main():
    ...
    for i, (sym, (name, exchange, ticker_str, currency, _div, hint_cur)) in enumerate(all_stocks.items()):
        ...
        out["stocks"][sym] = {
            ...
            "sector": CO_INFO.get(sym, {}).get("sector", "General")
        }
        out["stocks"][sym].update(arrs)
        out["annualisation"][sym] = ann

    # Compute sector medians
    sector_medians = compute_sector_medians(out["stocks"])
    out["sector_medians"] = sector_medians

    # Generate AI insights for each stock
    for sym, st_data in out["stocks"].items():
        ai_text = get_ai_insights(sym, st_data, sector_medians)
        st_data["ai_insight"] = ai_text
        st_data["ai_insight_date"] = NOW.isoformat()
