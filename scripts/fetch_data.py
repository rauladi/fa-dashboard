...
FISCAL_YEAR_END = {
    ...
    "AXP":12,
    "BAC":12,
    "ANZ":9,      # ANZ fiscal year ends September
    "AVGO":10,    # Broadcom fiscal year ends October
}
...
def annualise_year(tk, yr, div, fx, sym):
    ...
    # Fallback: use quarterly data to construct trailing twelve months (TTM)
    qi = tk.quarterly_financials
    qb = tk.quarterly_balance_sheet
    if qi is not None and not qi.empty:
        # Get all quarters sorted descending
        all_qtrs = sorted(qi.columns, reverse=True)
        if not all_qtrs:
            return {f: None for f in FIELDS}, {"method":"none","label":None}
        
        # For the requested year, find quarters that belong to that fiscal year
        fy_end_month = FISCAL_YEAR_END.get(sym, 12)
        target_qtrs = []
        for q in all_qtrs:
            q_year = q.year
            q_month = q.month
            # Fiscal year: if month <= fy_end_month, fiscal year = calendar year, else fiscal year = calendar year + 1
            fiscal_year = q_year if q_month <= fy_end_month else q_year + 1
            if fiscal_year == yr:
                target_qtrs.append(q)
        
        if target_qtrs:
            # Take up to 4 most recent quarters for that fiscal year
            ...
            # Balance sheet from most recent quarter in the set
            qbc = target_qtrs[0]
...
def fetch_one(sym, exchange, ticker_str, hint_cur, usd_aud, usd_idr, twd_usd):
    ...
    for attempt in range(2):
        try:
            ...
            # EPS and DPS fallback for IDX stocks
            if exchange == "IDX":
...
PROFILES = {
    # ... (keep your existing PROFILES dictionary) ...
    # I've omitted the full dictionary here for brevity, but it remains unchanged.
}

LEADERSHIP = {
    # ... (keep your existing LEADERSHIP dictionary) ...
}
