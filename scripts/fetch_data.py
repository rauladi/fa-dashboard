...
def find_row(df, *names):
    """Case‑insensitive row lookup using full index match after stripping whitespace."""
...
def annual_row(inc, bs, div, fx, sym, ic_col=None, bc_col=None):
    ...
    if inc is not None and ic_col is not None:
        ...
        # Exactly match Basic EPS or Diluted EPS as shown on Yahoo Finance
...
def fetch_one(sym, exchange, ticker_str, hint_cur, usd_aud, usd_idr, twd_usd):
    ...
    for attempt in range(2):
        try:
            ...
            # ---------- 2025 (LATEST_YEAR) ----------
            # Priority: annual TTM column → quarterly TTM → annual fiscal year → manual 4Q sum
            row_2025 = None
            method_2025 = None

            # 1) Try TTM column from annual financials (primary source, matches Yahoo "Annual" tab)
            ...
            # 3) Fallback to annual full‑year column for 2025 (if exists)
            ...
            # 4) Final fallback: manual sum of last 4 quarters (only if still missing)
            if row_2025 is None or row_2025.get("revenue") is None:
            ...
            # DPS from statistics page (always attempt after row construction)
...
PROFILES = { ... }   # ← your original full profiles (every stock) – unchanged
# ---------- LEADERSHIP (unchanged) ----------
LEADERSHIP = { ... } # ← your original leadership entries – unchanged
