...
def find_row(df, *names):
    """Case‑insensitive row lookup using stripped index."""
...
def get_ttm_col(df):
    """Return first column whose string representation (lowercased) contains 'ttm'."""
    if df is None or df.empty:
        return None
    for c in df.columns:
        col_str = str(c).lower()
...
def fetch_one(sym, exchange, ticker_str, hint_cur, usd_aud, usd_idr, twd_usd):
    ...
    for attempt in range(2):
        try:
            ...
            # ---------- 2025 (LATEST_YEAR): TTM from annual financials ----------
            row_2025 = None
            method_2025 = None

            # Priority 1: Annual TTM column
            ...
            # Priority 2: Quarterly TTM column (if annual TTM missing)
            ...
            # Priority 3: Full fiscal year column for 2025
            ...
            # Priority 4: Manual TTM sum of last 4 quarters (last resort)
            if row_2025 is None or row_2025.get("revenue") is None:
