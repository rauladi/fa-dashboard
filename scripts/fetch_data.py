...
def fetch_one_yahoo(sym, exchange, ticker_str, hint_cur, usd_aud, usd_idr, twd_usd):
    ...
    for attempt in range(2):
        try:
            ...
            # Fallback to Yahoo info if DPS missing
            if row_2025 is not None and row_2025.get("dps") is None:
                try:
                    info = tk.info
                    tdiv = info.get("trailingAnnualDividendRate")
                    if tdiv is not None and tdiv != 0:
                        row_2025["dps"] = round(tdiv * ps_fx, 4)
                except: pass

            # For known non‑payers, set DPS to None
