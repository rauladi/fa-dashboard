    ...
    for attempt in range(2):
        try:
            ...
            if qi is not None and not qi.empty:
                ...
                if all_qtrs:
                    # take up to the last 4 quarters
                    ttm_qtrs = all_qtrs[:4]
                    n = len(ttm_qtrs)
                    months = n * 3
                    label = f"{months}M"
                    ...
                    # Balance sheet: use latest quarter among the selected
                    if ttm_qtrs:
                        qbc = ttm_qtrs[0]   # most recent quarter
                        if qb is not None and not qb.empty and qbc in qb.columns:
                            ta = find_row(qb, "total assets", "totalassets")
                            ca = find_row(qb, "cash and cash equivalents", "cash", "cashandcashequivalents",
                                          "cash and short term investments", "cash & equivalents")
                    ...
                    # Scale income (revenue, gross, net) by div/fx – no annualization needed for TTM
                    for k in ["revenue", "grossProfit", "netProfit"]:
                    ...
                    print(f"      {LATEST_YEAR}: no quarterly data columns", flush=True)
            else:
                print(f"      {LATEST_YEAR}: quarterly data is empty", flush=True)

            # If quarterly TTM failed, try annual full year column (rare)
