...
# ---------- FALLBACK DATA (improved: 2025 EPS for NAB/ANZ) ----------
PRELOADED = {
...
def annualise_year(tk, yr, div, fx, sym):
    ...
    # 1) TTM from quarterly_financials for LATEST_YEAR
    if yr == LATEST_YEAR:
    ...
    # 2) Full fiscal year from annual financials
    ...
    # 3) TTM from quarterly (manual sum) for LATEST_YEAR
    if yr == LATEST_YEAR and qi is not None and not qi.empty:
        all_qtrs = sorted(qi.columns, reverse=True)
        if all_qtrs:
            fy_end_month = FISCAL_YEAR_END.get(sym, 12)
            target_qtrs = [q for q in all_qtrs if (q.year if q.month <= fy_end_month else q.year+1) == yr]
            if not target_qtrs:
    ...
    # 4) Generic quarterly for any year
    ...
    # 5) Last resort for LATEST_YEAR: previous year's annual data
    if yr == LATEST_YEAR:
        print(f"      No data for {yr}, using {yr-1} as estimate...", flush=True)
...
def fetch_one(sym, exchange, ticker_str, hint_cur, usd_aud, usd_idr, twd_usd):
    ...
    for attempt in range(2):
        try:
            ...
            # IDX EPS/DPS override
            if exchange == "IDX":
                for i, yr in enumerate(ALL_YEARS):
                    if yr in COMPLETED:
                        if sym in PRELOADED and "eps" in PRELOADED[sym] and i < len(PRELOADED[sym]["eps"]):
                            fb_eps = PRELOADED[sym]["eps"][i]
                            if fb_eps is not None:
                                yd[yr]["eps"] = fb_eps
                        if sym in PRELOADED and "dps" in PRELOADED[sym] and i < len(PRELOADED[sym]["dps"]):
                            fb_dps = PRELOADED[sym]["dps"][i]
                            if fb_dps is not None:
                                yd[yr]["dps"] = fb_dps

            # Fill any missing completed-year fields with non-None PRELOADED
            for yr in COMPLETED:
                year_index = ALL_YEARS.index(yr)
                for field in FIELDS:
                    if yd[yr].get(field) is None and sym in PRELOADED and field in PRELOADED[sym]:
                        if year_index < len(PRELOADED[sym][field]):
                            fb_val = PRELOADED[sym][field][year_index]
                            if fb_val is not None:
                                yd[yr][field] = fb_val
                                print(f"      using fallback {field} for {yr}: {fb_val}", flush=True)
...
# ---------- FULL PROFILES (original content unchanged) ----------
PROFILES = {
...
def main():
    ...
    for i, (sym, (name, exchange, ticker_str, currency, _div, hint_cur)) in enumerate(all_stocks.items()):
        if i > 0:
            time.sleep(1)
        # Per-stock error handling: if one fails, log and continue
        try:
            ...
            if yd is not None:
                ...
                arrs = build_arrays(None, sym)
                src = "fallback"
                ann = {"method":"none","label":None}
        except Exception as e:
            print(f"  [{sym}] Exception during fetch: {e}", flush=True)
            arrs = build_arrays(None, sym)
            src = "fallback"
            ann = {"method":"none","label":None}
