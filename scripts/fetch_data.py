...
# ---------- Direct Yahoo v10 balance sheet fallback ----------
def fetch_balance_sheet_v10(ticker_str):
    """Return dict with totalAssets, cash, totalDebt, totalEquity from Yahoo v10 API."""
    url = f"https://query2.finance.yahoo.com/v10/finance/quoteSummary/{ticker_str}?modules=balanceSheetHistoryQuarterly"
    ...
    try:
        ...
        if resp.status_code != 200:
            return {}
        data = resp.json()
        quotes = data.get("quoteSummary", {}).get("result", [])
        if not quotes:
            return {}
        bs_data = quotes[0].get("balanceSheetHistoryQuarterly", {}).get("balanceSheetStatements", [])
        if not bs_data:
            return {}
        latest = bs_data[0]
        out = {}
        # Map common fields
        for yahoo_key, our_key in [
            ("totalAssets", "totalAsset"),
            ("cashAndCashEquivalents", "cash"),
            ("totalDebt", "totalDebt"),
            ("totalStockholderEquity", "totalEquity"),
        ]:
            if yahoo_key in latest:
                out[our_key] = latest[yahoo_key]
        return out
    except Exception:
        return {}

# ---------- Yahoo Finance fetch (yfinance + fallbacks) ----------
def fetch_one_yahoo(sym, exchange, ticker_str, hint_cur, usd_aud, usd_idr, twd_usd):
    ...
    for attempt in range(2):
        try:
            ...
            # ---------- Balance‑sheet fallback: yfinance annual/quarterly, then v10 API ----------
            if row_2025 is not None:
                ...
                if bal_missing:
                    ...
                    # Final fallback: direct v10 API
                    if any(row_2025.get(f) is None for f in ("totalAsset","cash","totalDebt","totalEquity")):
                        print("  Trying Yahoo v10 balance sheet...", flush=True)
                        v10 = fetch_balance_sheet_v10(ticker_str)
                        if v10:
                            if row_2025.get("totalAsset") is None:
                                row_2025["totalAsset"] = safe(v10.get("totalAsset"), div, total_fx)
                            if row_2025.get("cash") is None:
                                row_2025["cash"] = safe(v10.get("cash"), div, total_fx)
                            if row_2025.get("totalDebt") is None:
                                row_2025["totalDebt"] = safe(v10.get("totalDebt"), div, total_fx)
                            if row_2025.get("totalEquity") is None:
                                row_2025["totalEquity"] = safe(v10.get("totalEquity"), div, total_fx)
                        ...
                        for s in src:
                            ...
                            if v is not None:
                                if k in ("eps", "dps"):
                                    ...
                                    row_2025[k] = safe(v, div, total_fx)
            ...
            print(f"  ✓ live years: {[y for y in yd if yd[y].get('revenue') is not None]}", flush=True)
...
def fetch_live(sym, exchange, ticker_str, hint_cur, usd_aud, usd_idr, twd_usd):
    ...
    return fetch_one_yahoo(sym, exchange, ticker_str, hint_cur, usd_aud, usd_idr, twd_usd)
...
def main():
    ...
    for i, (sym, (name, exchange, ticker_str, currency, _div, hint_cur)) in enumerate(all_stocks.items()):
        ...
        try:
            ...
            src = "fmp" if (USE_FMP and any(v is not None for r in yd.values() for v in r.values())) else "yahoo"
