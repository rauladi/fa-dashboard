...
print(f"Using FMP: {USE_FMP}", flush=True)
...
def financial_currency(exchange):
    """Return the true financial reporting currency for the exchange, ignoring any API profile."""
    if exchange == "IDX":
        return "IDR"
    if exchange == "ASX":
        return "AUD"
    return "USD"   # default for all others
...
def fetch_one_fmp(sym, ticker_str, target_cur, exchange, usd_aud, usd_idr, twd_usd):
    ...
    if inc_annual:
        ...
        row_2025["eps"]         = safe(eps_val, 1, ps_fx) if eps_val else None
        dps_val = stmt.get("dividendPerShare")
        row_2025["dps"]         = safe(dps_val, 1, ps_fx) if dps_val else None
    if bal_annual:
        stmt = bal_annual[0]
        row_2025["totalAsset"]  = safe(stmt.get("totalAssets"), div, total_fx)
        row_2025["cash"]        = safe(stmt.get("cashAndCashEquivalents"), div, total_fx)
...
# ---------- PROFILES ----------
PROFILES = { ... }  # as previously provided – full text

# ---------- LEADERSHIP ----------
LEADERSHIP = { ... }  # as previously provided – full text
