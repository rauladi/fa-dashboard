...
# ── Fiscal Year end months (for display info & completeness guard) ──────────
# month = last month of fiscal year (e.g. 6 = Jun 30, 12 = Dec 31, 1 = Jan 31)
FISCAL_YEAR_END = {
    "BHP": 6, "WDS": 12, "CBA": 6,                           # ASX
    "BBRI":12,"ADRO":12,"SMSM":12,"UNTR":12,                 # IDX Dec
    "ITMG":12,"POWR":12,"MPMX":12,"BTPS":12,"DMAS":12,"SPTO":12,
    "TSM": 12,"V": 9, "MA": 12,                               # NYSE
    "MSFT": 6,"AMZN":12,"AAPL": 9,"META":12,"NVDA": 1,       # NASDAQ (NVDA=Jan)
    "GOOG":12,"BKNG":12,
}
...
def load_config_stocks():
    ...
    try:
        ...
        for s in cfg.get("custom_stocks", []):
            ...
            print(f"  [config] Extra stock: {sym}", flush=True)
        return extra
...
# ── Exchange rates ────────────────────────────────────────────────────────
def get_rates():
    # Fallbacks: 1 USD = ~1.58 AUD, 1 USD = ~16300 IDR, 1 TWD = ~0.031 USD
    ...
    try:
        ...
        if not h.empty:
            ...
            if 0.50 < v < 0.95:
                usd_aud = round(1.0/v, 6)  # AUDUSD=X gives USD per AUD → invert for AUD per USD
        print(f"  USD→AUD: {usd_aud:.4f}  (1 USD = {usd_aud:.4f} AUD)", flush=True)
    except Exception as e:
    ...
    try:
        ...
        if not h.empty:
            v = float(h["Close"].iloc[-1])
            if 10000 < v < 25000:
                usd_idr = round(v, 0)  # IDR=X gives IDR per USD directly
        print(f"  USD→IDR: {usd_idr:.0f}  (1 USD = {usd_idr:.0f} IDR)", flush=True)
    except Exception as e:
    ...
    try:
        ...
        if not h.empty:
            v = float(h["Close"].iloc[-1])
            if 0.01 < v < 0.10:
                twd_usd = round(v, 6)  # TWDUSD=X gives USD per TWD
...
def get_fx(exchange, fin_cur, usd_aud, usd_idr, twd_usd):
    """Return (divisor, fx_multiplier) to convert raw yfinance values → display units.
    
    Currency logic:
    - ASX display in B AUD:
        BHP/WDS report in USD → multiply by usd_aud (USD→AUD)
        CBA reports in AUD    → multiply by 1.0
    - IDX display in T IDR:
        BBRI/SMSM etc report in IDR → multiply by 1.0
        ADRO/ITMG/POWR report in USD → multiply by usd_idr (USD→IDR)
    - NYSE/NASDAQ display in B USD:
        Most report in USD → multiply by 1.0
        TSM reports in TWD → multiply by twd_usd (TWD→USD)
    """
    if exchange == "ASX":
        ...
        fx = twd_usd if fin_cur == "TWD" else 1.0
        return 1e9, fx

def eps_fx(exchange, fin_cur, usd_aud, usd_idr, twd_usd):
    """FX for per-share values (EPS/DPS — no divisor, currency conversion only)."""
...
def col_yr(df, yr):
    """Return the most recent annual column for fiscal year `yr`.
    Matches any column whose .year attribute == yr (handles non-Dec FY ends)."""
...
def annual_row(inc, bs, cf, yr, div, fx, epsfx, sym=""):
    ...
    if ic is not None:
        ...
        # Only flag as incomplete if EPS is missing while revenue exists.
        # This catches TTM/preliminary data that lacks per-share figures.
        # We do NOT restrict by column month, because valid FY ends occur in
        # all months: Jun (CBA,MSFT), Sep (AAPL,V), Jan (NVDA), Dec (most).
        if yr == LATEST_YEAR and rev_val is not None and eps_val is None:
            fy_end = FISCAL_YEAR_END.get(sym, 12)
            # Allow if column month matches this stock's known FY end month
            # (i.e. it's a real annual report, just EPS not yet filed)
            col_month = ic.month
            if col_month != fy_end:
                print(f"    ⚠ {sym} yr={yr}: col={ic.date()} FY_end_month={fy_end} "
                      f"col_month={col_month} → EPS missing, interim/TTM, skipping", flush=True)
                ...
                print(f"    ℹ {sym} yr={yr}: col={ic.date()} matches FY_end, accepting despite no EPS",
                      flush=True)
    ...
    if cc is not None and row.get("_sh"):
        dp = find_row(cf,"Cash Dividends Paid","Dividends Paid","Common Stock Dividend Paid",
...
def current_year_row(tk, yr, div, fx, epsfx):
    ...
    try:
        ...
        sh = find_row(qi,"Basic Average Shares","BasicAverageShares","Diluted Average Shares",
                      "Average Dilution Earnings")
        ...
        row["revenue"]    = af(rv); row["grossProfit"] = af(gp)
        row["netProfit"]  = af(ni); row["eps"]         = ae(ep)
        ...
        if qc is not None and not qc.empty and sh_val:
            cq  = cols_yr(qc, yr)
            dp  = find_row(qc,"Cash Dividends Paid","Dividends Paid","Common Stock Dividend Paid",
                           "Payment Of Dividends")
            ytd = sum_q(dp, cq)
            row["dps"] = round(abs(ytd)/sh_val*epsfx*factor, 4) if ytd and sh_val > 0 else None

        # FCF (annualised from quarterly OCF + CapEx)
        if qc is not None and not qc.empty:
            cq  = cols_yr(qc, yr)
            op  = find_row(qc,"Operating Cash Flow","Cash From Operations",
                           "Total Cash From Operating Activities","Net Cash Provided By Operating Activities")
            cx  = find_row(qc,"Capital Expenditure","Capital Expenditures","Purchase Of Ppe",
                           "Purchases Of Property Plant And Equipment")
            op_sum = sum_q(op, cq); cx_sum = sum_q(cx, cq)
            if op_sum is not None:
                op_ann = round(op_sum/div*fx*factor, 4)
                cx_ann = round(cx_sum/div*fx*factor, 4) if cx_sum is not None else 0
                row["fcf"] = round(op_ann + cx_ann, 4)
...
def fetch_one(sym, exchange, ticker_str, hint_cur, usd_aud, usd_idr, twd_usd):
    ...
    for attempt in range(2):
        try:
            ...
            print(f"  cur={fin_cur} div=1e{int(math.log10(div))} "
                  f"fx={fx:.6f} epsfx={epsfx:.6f}", flush=True)

            inc = tk.financials; bs = tk.balance_sheet; cf = tk.cashflow
            if inc is None or inc.empty: raise ValueError("no annual data")

            # Print available column dates for debugging
            print(f"  Annual cols: {[str(c.date()) for c in inc.columns if hasattr(c,'date')]}",
                  flush=True)
            ...
            for yr in COMPLETED[-2:]:
                ...
                if ta: print(f"  {yr}: asset={ta:.2f} rev={rv} fcf={yd[yr].get('fcf')}",
                             flush=True)
...
def main():
    ...
    out = {
        ...
        "rates": {
            "usdToAud": usd_aud,   # 1 USD = X AUD
            "usdToIdr": usd_idr,   # 1 USD = X IDR
            "twdToUsd": twd_usd,   # 1 TWD = X USD
            "audToUsd": round(1.0/usd_aud, 6),   # 1 AUD = X USD (informational)
            "idrToUsd": round(1.0/usd_idr, 9),   # 1 IDR = X USD (informational)
        },
    ...
    for i, (sym, (name, exchange, ticker_str, currency, _div, hint_cur)) in enumerate(all_stocks.items()):
        ...
        out["stocks"][sym] = {
            ...
            "fyEndMonth": FISCAL_YEAR_END.get(sym, 12)
        }
